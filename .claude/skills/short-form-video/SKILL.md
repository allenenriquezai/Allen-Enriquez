...
## Fast overlay iteration SOP — Chrome headless PNG + ffmpeg

When iterating on hook sticker copy, a pinned header, or any text overlay on an ALREADY-RENDERED reel, do NOT re-render the full Hyperframes composition. Screenshot-capture mode takes ~8 minutes per pass. Use this path instead (takes <30 seconds):
...
## Face + animation compositing SOP (when renderer fails to embed face)
...
- `projects/personal/videos/reel-3/` — the hook-sticker + spy-avatar + ffmpeg-composite pattern. Shows the "face-under, animation-over, sticker-on-top" three-input ffmpeg path end-to-end.
...
- `feedback_hook_sticker_style.md` — hook sticker spec (dark pill + cyan border, white uppercase)
- `feedback_captions_style.md` — karaoke captions spec (Montserrat 900 68px, layered stroke, blue active word)
- `feedback_overlay_png_chrome_headless.md` — the Chrome-headless + ffmpeg fast path for text overlay iteration
