...
$PYTHON tools/personal/outreach.py enrich --limit 20 >> "$LOG" 2>&1
$PYTHON tools/personal/outreach.py followups >> "$LOG" 2>&1
$PYTHON tools/personal/outreach.py queue >> "$LOG" 2>&1
$PYTHON tools/personal/outreach.py replies >> "$LOG" 2>&1

QUEUE_FILE="projects/personal/.tmp/outreach_queue_${DATE}.md"
if [ -f "$QUEUE_FILE" ]; then
    COUNT=$(grep -c "^## [0-9]" "$QUEUE_FILE" 2>/dev/null || echo 0)
    MSG="PH outreach ready: $COUNT messages for $DATE. Open $QUEUE_FILE, send, then run: python3 tools/personal/outreach.py log-sent --ids 1,2,3..."
    $PYTHON tools/eps/whatsapp.py --body "$MSG" >> "$LOG" 2>&1 || echo "[$(date)] whatsapp notify failed" >> "$LOG"
fi
