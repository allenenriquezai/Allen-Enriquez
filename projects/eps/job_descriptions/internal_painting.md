# Internal Painting - Job Description Template

<!-- source: Google Drive -->
<!-- quote_title: INTERNAL PAINTING QUOTE -->

JOB SUMMARY
* [Client situation – rewritten professionally if provided.]
* Internal surfaces prepared for repaint or new construction coating.
* Objective is a consistent, durable finish using a Dulux interior paint system applied to manufacturer specifications.

---

1. PROJECT DETAILS & MEASUREMENTS
* Wall linear length: [___] LM
* Wall surface area: [___] m²
* Floor area: [___] m²
* Ceiling height: [Standard / High]

---

2. SCOPE OF WORK

REPAINT WORKS
* Minor surface preparation including sanding and gap filling where required
* Spot priming of exposed areas where required
* Application of two full finishing coats of Dulux interior paint system

NEW CONSTRUCTION WORKS
* Surface inspection prior to commencement
* Application of one full primer coat
* Application of two full finishing coats
* Three coat system applied in accordance with Dulux specifications
* Minor preparation only, limited to light sanding and minor touch-ups

* Painting of all internal areas listed below as specified
* Final inspection and thorough clean-up of work areas

---

3. PREPARATION & EXECUTION
* Protection of floors, fixtures, cabinetry, windows, and fittings
* Surface preparation completed in accordance with Dulux manufacturer recommendations
* Brush, roller, and or spray application for consistent coverage
* Correct drying times observed between coats as per Dulux specifications
* Care taken to minimise disruption during works

---

4. INTERNAL AREAS
* Internal wall painting
* Internal ceiling painting
* Internal skirting board painting
* Internal door painting both sides
* Timber trims, frames, and architraves where applicable

OPTIONAL LEVEL BREAKDOWN FOR NEW CONSTRUCTION
A) GROUND FLOOR
* Internal wall painting
* Internal ceiling painting
* Internal skirting board painting
* Internal door painting both sides
B) FIRST FLOOR
* Internal wall painting
* Internal ceiling painting
* Internal skirting board painting
* Internal door painting both sides
C) STAIRWELL OR ADDITIONAL AREAS WHERE APPLICABLE
* Wall and ceiling painting

---

5. INCLUSIONS
* Labour for all internal painting works
* Minor preparation and patching
* All preparation, priming, and coating materials
* Paint system: Dulux premium interior paint
* Professional-grade equipment
* Site protection and clean-up
* After-completion photographs provided upon request

---

6. PAINT PRODUCTS, WARRANTIES & LICENSING
* Dulux premium interior paint systems engineered for durability, washability, and long-term finish performance
* Dulux manufacturer product warranties apply when coatings are applied in accordance with specifications
* EPS Paint provides a Fifteen Year Workmanship Warranty covering application, adhesion, and finish quality
* EPS Paint operates with full QBCC licensing and insurance
* All works completed to Australian Standards and industry best practice

---

7. GENERAL EXCLUSIONS
* Asbestos handling or removal
* Major plaster repairs, stopping rectification, or surface correction beyond minor preparation
* For new construction works, surfaces are assumed to be delivered paint-ready by others; rectification beyond minor preparation will be treated as a variation
* Structural rectification works
* Silicone or Sika-Flex usage
* Damage caused by other trades after our attendance
* Areas not specifically listed in the Scope of Work

---

8. BOOKING & PAYMENT TERMS
* A thirty percent deposit is required prior to commencement
* Remaining balance payable upon practical completion
* Variations outside the agreed Scope of Work will be quoted separately and require written approval prior to execution
* Minimum fourteen days advance booking required

---

## Personalisation Prompts
- Client situation (who they are, why they're getting this done)
- Emotional drivers (e.g. selling, new home, builder handover pressure)
- Any scope specifics not covered by standard inclusions
