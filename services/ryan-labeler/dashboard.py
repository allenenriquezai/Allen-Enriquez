...
def _nav_tabs_html(token: str) -> str:
    def _tab(label: str, href: str, active: bool) -> str:
        cls = "dtab active" if active else "dtab"
        return f'<a class="{cls}" href="{href}?token={token}">{label}</a>'
    return (
        '<div class="tabs-bar">'
        + _tab("Dashboard", "/dashboard", True)
        + _tab("Morning Brief", "/brief-preview", False)
        + _tab("Evening Brief", "/evening-brief-preview", False)
        + '</div>'
    )
...
def render_dashboard(token: str = "ryan-sc") -> str:
    ...
    tabs_html      = _nav_tabs_html(token)
